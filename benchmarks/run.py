#!/usr/bin/env python3
"""Benchmark S.H.E.L.L. vs normal agent output token counts."""
# Rename shell references to shell
def run_benchmark():
    print("Running S.H.E.L.L. benchmark...")
...
SKILL_PATH = REPO_DIR / "skills" / "shell" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, shell_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("shell", shell_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        shell_medians = statistics.median(
            [t["output_tokens"] for t in entry["shell"]]
        )
        savings = 1 - (shell_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "shell_median": int(shell_medians),
    ...
    avg_shell = round(statistics.mean([r["shell_median"] for r in rows]))
...
def format_table(rows, summary):
    ...
    for r in rows:
        ...
        lines.append(
            f"| {label} | {r['normal_median']} | {r['shell_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_shell']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark shell vs normal Claude")
    ...
    shell_system = load_shell_system()
    ...
    results = run_benchmarks(client, args.model, prompts, shell_system, args.trials)
